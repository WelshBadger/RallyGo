...
export default function ManageEventPage() {
  ...
  async function handleUpload(e) {
    ...
    try {
      ...
      // Rally Guide PDFs: extract regulations-format data only.
      // Do NOT run extract-final-instructions here — that would overwrite any real FI
      // data that the organiser already uploaded, which contains more authoritative times.
  ...
  function parseCSV(text) {
    ...
    const MAP = {
      ...
      codriver: ['co-driver', 'codriver', 'co driver', 'navigator', 'driver 2'],
      class: ['class', 'category', 'cat', 'group'],
      vehicle: ['vehicle', 'car make', 'make', 'model', 'car model', 'make/model'],
  ...
  function handleEntryFileChange(e) {
  ...
  async function clearEntryList() {
    if (!confirm('Clear the entry list?')) return
    await supabase.from('rallies').update({ entry_list_data: null }).eq('id', rallyId)
    setRally(r => ({ ...r, entry_list_data: null }))
    toast.success('Entry list cleared')
  }
  ...
  async function handleStageMapUpload(e) {
    ...
    setStageMapUploading(true)
    try {
      const ext = file.name.split('.').pop()
      const path = `${rallyId}/stagemap_${Date.now()}.${ext}`
      const { error: uploadErr } = await supabase.storage.from('rally-docs').upload(path, file, { contentType: file.type, upsert: true })
      if (uploadErr) throw uploadErr
      const { data: { publicUrl } } = supabase.storage.from('rally-docs').getPublicUrl(path)
  ...
  async function removeStageMap(id) {
    const next = (rally.stage_maps || []).filter(m => m.id !== id)
    await supabase.from('rallies').update({ stage_maps: next }).eq('id', rallyId)
    setRally(r => ({ ...r, stage_maps: next }))
  }
  ...
  return (
    <main className="max-w-4xl mx-auto px-4 py-6">
      ...
      {/* Stage maps & route card — labelled list of images / PDFs */}
      <div className="bg-rl-card border border-white/10 rounded-xl p-5 mb-5">
        <div className="mb-4">
          <h2 className="text-white font-medium text-sm">Stage maps & route</h2>
          <p className="text-white/35 text-xs mt-0.5">Add stage maps and route diagrams — images or PDFs, each with a label. Add as many as you like. They appear in every crew's Rally Logistics Stages tab.</p>
        </div>

        {/* Existing items */}
        {rally.stage_maps?.length > 0 && (
          <div className="space-y-2 mb-4">
            {rally.stage_maps.map(m => (
        ...
        <div className="flex flex-col sm:flex-row gap-2 items-stretch sm:items-center">
          <input
            ...
            placeholder="Label — e.g. SS1 map, Route overview, Service diagram"
          ...
          <label className="rl-btn-primary text-xs cursor-pointer justify-center py-2.5 flex-shrink-0">
            {stageMapUploading ? 'Uploading…' : '+ Add image / PDF'}
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          {rally.entry_list_data?.length > 0 && (
            <div className="flex items-center gap-3 flex-shrink-0">
              <span className="text-white/40 text-xs">{rally.entry_list_data.length} entries</span>
              <button onClick={clearEntryList} className="text-xs text-red-400/60 hover:text-red-400 transition-colors">Clear</button>
            </div>
          )}
        </div>
              ...
              <table className="w-full text-xs">
                <thead>
                  <tr className="border-b border-white/10 bg-white/3">
                    {['#', 'Driver', 'Co-driver', 'Class', 'Vehicle'].map(h => (
                      <th key={h} className="text-left px-3 py-2 text-white/35 font-medium whitespace-nowrap">{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody className="divide-y divide-white/5">
                  {rally.entry_list_data.slice(0, 5).map((e, i) => (
                    <tr key={i}>
                      ...
                      <td className="px-3 py-2 text-white/60">{e.vehicle}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
            </div>
            {rally.entry_list_data.length > 5 && (
              <p className="text-white/25 text-xs px-3 py-2 border-t border-white/8">+{rally.entry_list_data.length - 5} more entries</p>
            )}
          </div>
        )}
